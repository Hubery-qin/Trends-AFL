...
u64   tmp_pilot_time = 0, /* 轮回次数 */
      ...
      total_wheel = 1, /*总轮数*/
      adjust_gap = 24*60*60*1000, /* 改变的时间间隔*/
      bingo_time = 0;

u8    negative_feedback = 0, /* 负反馈计数 */
...
static u64  operator_count_stack[operator_num];  /*堆栈算子选择次数计数*/
double  operator_count_per_round[wheel_num][operator_num],   /*每轮算子选择次数计数*/
        interesting_count_per_round[wheel_num][operator_num], /*每轮算子有趣值计数*/
        interesting_total_conut_per_round[wheel_num],  /*每轮总有趣值计数*/
        fitness[wheel_num][operator_num], /*适应度值*/
        fitness_avg[operator_num], /*各算子的平均适用度值*/
        prod[wheel_num][operator_num+2], /*概率*/
        weight_function[affected_wheels+1]; /*权重函数*/
...
static void record_experimental_data() {
  ...
  fprintf(experimental_data, 
          "当前时间: %llu, 轮回次数: %llu, 每轮新增: %llu, total: %llu,\
          choose: %lf, interesting:  %lf,  fitness: %lf, fitness_avg: %lf, prod: %lf, \
          choose: %lf, interesting:  %lf,  fitness: %lf, fitness_avg: %lf, prod: %lf, \
          choose: %lf, interesting:  %lf,  fitness: %lf, fitness_avg: %lf, prod: %lf, \
          choose: %lf, interesting:  %lf,  fitness: %lf, fitness_avg: %lf, prod: %lf, \
          choose: %lf, interesting:  %lf,  fitness: %lf, fitness_avg: %lf, prod: %lf, \
...
static void initialization() {
  ...
  // 初始化 权重函数
  if (weight_function[0] != 1)
  {
    for (i = 0; i <= affected_wheels; i++)
    {
      weight_function[i] = pow(e,-(i/pow(e,2)));
...
static void follow_updating() {
  ...
  for (i = 0; i < operator_number; i++)
  {
    ...
    for (j = 0; j < affected_wheels && j < cycle_wheel; j++) // 侦察蜂 后开始算 一轮
    {
      ...
      fitness[current_wheel][i] += weight_function[j+1] * temp_rand * interesting_count_per_round[k][i] / operator_count_per_round[k][i];
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    for (i = 0; i < use_stacking; i++) {
      ...
      // switch (UR(15 + ((extras_cnt + a_extras_cnt) ? 2 : 0))) {
						case 0:
							/* Flip a single bit somewhere. Spooky! */
							FLIP_BIT(out_buf, UR(temp_len << 3));
							operator_count_stack[STAGE_FLIP1] += 1;
							break;
						...
						case 1:
							...
							operator_count_stack[STAGE_FLIP2] += 1;
							break;
						...
						case 2:
							...
							operator_count_stack[STAGE_FLIP4] += 1;
							break;

						case 3:
							if (temp_len < 4) break;
							out_buf[UR(temp_len)] ^= 0xFF;
							operator_count_stack[STAGE_FLIP8] += 1;
							break;

						case 4:
							if (temp_len < 8) break;
							*(u16*)(out_buf + UR(temp_len - 1)) ^= 0xFFFF;
							operator_count_stack[STAGE_FLIP16] += 1;
							break;

						case 5:
							if (temp_len < 8) break;
							*(u32*)(out_buf + UR(temp_len - 3)) ^= 0xFFFFFFFF;
							operator_count_stack[STAGE_FLIP32] += 1;
							break;

						case 6:
							out_buf[UR(temp_len)] -= 1 + UR(ARITH_MAX);
							out_buf[UR(temp_len)] += 1 + UR(ARITH_MAX);
							operator_count_stack[STAGE_ARITH8] += 1;
							break;
						...
						case 7:
							...
							operator_count_stack[STAGE_ARITH16] += 1;
							break;
						...
						case 8:
							...
							operator_count_stack[STAGE_ARITH32] += 1;
							break;
						...
						case 9:
							...
							if (temp_len < 4) break;
							out_buf[UR(temp_len)] = interesting_8[UR(sizeof(interesting_8))];
							operator_count_stack[STAGE_INTEREST8] += 1;
							break;
						...
						case 10:
							...
							operator_count_stack[STAGE_INTEREST16] += 1;
							break;
						...
						case 11:
							...
							operator_count_stack[STAGE_INTEREST32] += 1;
							break;
						...
						case 12:
							...
							out_buf[UR(temp_len)] ^= 1 + UR(255);
							operator_count_stack[STAGE_RANDOMBYTE] += 1;
							break;
						...
						case 13: {
							...
							temp_len -= del_len;
							operator_count_stack[STAGE_DELETEBYTE] += 1;
							break;
						...
						case 14:
							...
							if (temp_len + HAVOC_BLK_XL < MAX_FILE) {
								...
								out_buf = new_buf;
								temp_len += clone_len;
								operator_count_stack[STAGE_Clone75] += 1; 
							}
						...
						case 15: {
							...
							if (UR(4)) {
								...
								UR(2) ? UR(256) : out_buf[UR(temp_len)], copy_len);
							operator_count_stack[STAGE_OverWrite75] += 1; 
							break;
            ...
            case 16: {
                ...
                operator_count_stack[STAGE_OverWriteExtra] += 1;
            ...
            case 17: {
              ...
              out_buf   = new_buf;
              temp_len += extra_len;
              operator_count_stack[STAGE_InsertExtra] += 1;
              break;
    ...
    if (unlikely(newly_added > 0 ))
    {
      ...
      for(k =0; k< operator_num; k++){
        if ( operator_count_stack[k] > 0 )// 是本次堆栈中的算子
        { // 本轮算子有趣值 计数
          interesting_count_per_round[current_wheel][k] += newly_added;
        }
        // fprintf(operator_out, "%d,", operator_count_stack[k] > 0? newly_added : 0);
      } 
    ...
    for(k =0;k< operator_num;k++){
      ...
      operator_count_per_round[current_wheel][k] += operator_count_stack[k];
      operator_count_stack[k] = 0;
    }
